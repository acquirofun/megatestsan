...
const Partners = () => {
  ...
  const performTask = (taskId) => {
    const task = tasks.advertTasks.find(task => task.id === taskId);
    if (task) {
        ...
        if (newCountdown <= 0) {
          ...
          const saveTaskToUser = async () => {
            ...
            try {
              ...
              const response = await fetch(`${SERVER_URL}/api/save-advert-task`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                  ...
                  advertTasksCompleted: { taskId: taskId, completed: false }
                }),
              });
...
try {
  const task = tasks.advertTasks.find(task => task.id === taskId);
  const SERVER_URL = process.env.REACT_APP_SERVER_URL;
  const response = await fetch(`${SERVER_URL}/api/claim-advert-task`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      telegramId: user.telegramId,
      advertTasksCompleted: { taskId, completed: true },
  ...
  setUser(prevUser => ({
    ...
    advertTasksCompleted: data.data.advertTasksCompleted
  }));
  ...
  setModalMessage(
    ...
    <h3 className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2">
      <span className={`text-accent`}>+{formatNumber(task.bonusAmount)}</span> NGT CLAIMED
    </h3>
  ...
  useEffect(() => {
    const submittedStates = tasks.advertTasks.reduce((acc, task) => {
...
{tasks.advertTasks
  ...
  .map(task => {
    const userTasks2 = user?.advertTasksCompleted || [];
    ...
    const isTaskSaved = !!userTask;
        ...
        return (
          <div key={task.id} className="w-full bg-[#17181A] px-3 rounded-[14px] py-3 flex items-center justify-between space-x-1">
                  ...
                  {!isTaskSaved && !isTaskCompleted && (
                    <>
                      <button
                        ...
                        className={`${showVerifyButtons[task.id] ? '!w-[45px] py-[10px] !px-[6px] text-[12px] !rounded-[12px]' : ''} w-[78px] py-[10px] text-center font-semibold rounded-[30px] px-3 bg-[#36373c] text-[#fff]`}
                      >
                        Start
                      </button>
