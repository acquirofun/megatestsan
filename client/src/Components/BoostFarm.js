...
const BoostFarm = () => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response2 = await axios.post(`${SERVER_URL}/api/claim-farming-boost`, {
      ...
      setUser(prevUser => ({
        ...prevUser,
        miningPower: response2.data.data.miningPower,
        tonTransactions: response2.data.data.tonTransactions
      }));
      ...
      setMessage(
        ...
        error.message || 
        "Transaction failed or canceled, please try again later."
      );
      setMessageColor("red");
    } finally {
