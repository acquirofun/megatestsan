...
const YouTubeTasks = () => {
    const {user, setUser, tasks} = useMongoUser();
    ...
    const userYoutubeTasks = user?.youtubeTasksCompleted;

    const performTask = (taskId) => {
        const task = tasks.youtubeTasks.find(task => task.id === taskId);
        if (task) {
            ...
            setCountdowns(prevCountdowns => {
                ...
                if (newCountdown <= 0) {
                    ...
                    const saveTaskToUser = async () => {
                        try {
                            const updatedUserYoutubeTasks = [...user.youtubeTasksCompleted, { taskId: taskId, completed: false }];

                            const SERVER_URL = process.env.REACT_APP_SERVER_URL;
                            const response = await fetch(`${SERVER_URL}/api/save-youtube-task`, {
                              method: 'POST',
                              headers: {
                              ...
                              body: JSON.stringify({
                                ...
                                youtubeTasksCompleted: { taskId: taskId, completed: false }
                              }),
                            });
                            ...
                            setUser(prevUser => ({
                                ...prevUser,
                                youtubeTasksCompleted: updatedUserYoutubeTasks
                              }));
    ...
    const claimTask = async (taskId) => {
        ...
        try {
            ...
            const SERVER_URL = process.env.REACT_APP_SERVER_URL;
            const response = await fetch(`${SERVER_URL}/api/claim-youtube-task`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({
                telegramId: user.telegramId,
                youtubeTasksCompleted: { taskId, completed: true },
            ...
            if (!data.success) {
              throw new Error(data.message || 'Failed to claim youtube task');
            }
            ...
            setUser(prevUser => ({
              ...
              youtubeTasksCompleted: data.data.youtubeTasksCompleted
            }));
            setModalMessage(
                <>
                <div className="w-full flex justify-center flex-col items-center space-y-3">
                    ...
                    <h3 className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2">
                        <span className="text-btn4">+{formatNumber(task.bonusAmount)}</span> {texts.projectSymbol} CLAIMED
                    </h3>
            ...
            console.error('Error claiming manual task:', err);
          } finally {
        ...
        //             </div>
        //             <h3 className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2">
        //                 <span className="text-btn4">+{formatNumber(task.bonus)}</span> ALDOGS CLAIMED
        //             </h3>
        //             <p className="pb-6 text-[15px] w-full text-center">
        //                 Keep performing new tasks! something huge is coming! Perform more and earn more ALDOGS now! 
        //             </p>
        //         </div>
        //         <div className="w-full flex justify-center">
        //     <button
    ...
    useEffect(() => {
        const submittedStatesYt = tasks.youtubeTasks.reduce((acc, task) => {
    ...
    return (
        <>
            {tasks.youtubeTasks
                .sort((a, b) => a.id - b.id)
                .map(task => {
                    const userTasks2 = user?.youtubeTasksCompleted || [];
                    const userTask = userTasks2.find(t => t.taskId === task.id);
             ...
             <IoClose size={20} className="text-[#9995a4]"/>
                    ...
                    <div className="bg-cards rounded-[10px] p-[14px] flex items-center justify-between mb-3">
                        <div className='flex flex-1 items-center space-x-1'>
                            ...
                            <div className="flex items-center space-x-1">
                                ...
                                <span className="font-semibold">+{formatNumber(task.bonusAmount)}</span>
                            </div>
                        </div>
                    ...
                    <div className="bg-cards rounded-[10px] p-[14px] flex flex-col space-y-2 justify-center items-center">
                        <div className="flex justify-center items-center relative">
                            <img onClick={() => performTask(task.id)} src={task.thumbnail} alt='thumb' className='w-full aspect-video rounded-[8px]' />
