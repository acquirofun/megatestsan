...
const ManualTasks = () => {
  ...
  const performTask = (taskId) => {
    const task = tasks.manualTasks.find(task => task.id === taskId);
    if (task) {
        ...
        if (newCountdown <= 0) {
          ...
          const saveTaskToUser = async () => {
            ...
            try {
              ...
              const response = await fetch(`${SERVER_URL}/api/save-manual-task`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                  ...
                  manualTasksCompleted: { taskId: taskId, completed: false }
                }),
              });
...
try {
  const task = tasks.manualTasks.find(task => task.id === taskId);
  const SERVER_URL = process.env.REACT_APP_SERVER_URL;
  const response = await fetch(`${SERVER_URL}/api/claim-manual-task`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      telegramId: user.telegramId,
      manualTasksCompleted: { taskId, completed: true },
  ...
  setUser(prevUser => ({
    ...
    manualTasksCompleted: data.data.manualTasksCompleted
  }));
  ...
  useEffect(() => {
    const submittedStates = tasks.manualTasks.reduce((acc, task) => {
...
{tasks.manualTasks
  ...
  .map(task => {
    const userTasks2 = user?.manualTasksCompleted || [];
