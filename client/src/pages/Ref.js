...
const Ref = () => {
  const { user, referrals, loading } = useMongoUser();
  const [copied, setCopied] = useState(false);
  const [info, setInfo] = useState(false)

  const totalBalance = referrals.reduce((sum, ref) => sum + ref.balance, 0);
  const referralBonus = (totalBalance * 0.05); // 20% of the total balance
  ...
  const copyToClipboard = () => {
    ...
    const reflink = `https://t.me/${urls.botUsername}/dogs?startapp=${user.telegramId}\n\ ${texts.projectSymbol} tokens mining is live! Two is better than one!  Join my squad, and let\'s double the fun (and earnings 🤑)! ${texts.projectSymbol} Power Tap! 🚀`;
  ...
  const handleShare = async () => {
    const shareData = {
      title: `Mine ${texts.projectName} tokens now!`,
      // eslint-disable-next-line
      url: `https://t.me/${urls.botUsername}/dogs?startapp=${user.telegramId}\n\ `, // Replace with your link
      text: `${texts.projectName} tokens mining is live! Two is better than one!  Join my squad, and let's double the fun (and earnings 🤑)! ${texts.projectName} Power Tap! 🚀`,
    };
  ...
  const fallbackShare = (shareData) => {
    const encodedText = encodeURIComponent(
      // eslint-disable-next-line
      `${shareData.url} \n\ ${texts.projectName} tokens mining is live! Two is better than one!  Join my squad, and let\'s double the fun (and earnings 🤑)! ${texts.projectName} Power Tap! 🚀`
    );
          ...
          <div className="w-full pt-3 justify-center flex-col space-y-3 px-5">
            <div className="w-full flex justify-center">
              <h1 className="font-semibold text-[28px] text-[#ffffffe0] pb-1 text-center">
                Invite friends <br/>& get more {texts.projectSymbol}
              </h1>
            </div>
...
<div className="flex items-center space-x-3">
  ...
  <div className="flex flex-col">
    ...
    <p className="text-[13px]">
      get +200 {texts.projectSymbol} & 5% of your friend's earnings
    </p>
  </div>
</div>
    ...
    <p className="text-[13px]">
      +100 to you and your friend
            ...
            <div className="w-full flex justify-between items-center">
              ...
              >
             <span className="">
                {formatNumber(referralBonus)} {texts.projectName}
                </span>  
