...
import { ErrorDisplay } from '../Components/LoadingSpinner';
...
const GoldHunters = () => {
  ...
  const { user, loading, loadingTwo, checker, setChecker, setLoadingTwo, error, authenticateUser } = useMongoUser();
...
if (error) {
  return <ErrorDisplay message={error} onRetry={authenticateUser} />;
}
...
<div className='w-full relative flex flex-col px-6 pt-6 items-center'>
        ...
        <img src={images.logo} alt='stars' className='w-[60px]'/>
...
{checker && (
      ...
      <div className="w-full flex h-full taskbg mt-[2px] justify-center relative flex-col items-center px-4">
        ...
        <div className='absolute w-[180px] h-[180px]'>
          ...
          <img src={images.logo} alt='gods' className='w-full'/>
        ...
        <div className='absolute bottom-10 flex flex-col w-full justify-center items-center space-y-5'>
          ...
          <p className='w-full text-center max-w-[20em]'>
           Welcome to {texts.projectName}!<br/> Start mining {texts.projectName} tokens today!
          </p>
            {user.balance > 0 ? (
