...
import { MongoUserProvider } from "../context/mongoContext";
...
useEffect(() => {
  ...
  return () => {
    ...
    useEffect(() => {
      ...
      const isTelegramApp = tele && tele?.initDataUnsafe && window.Telegram?.WebApp?.initDataUnsafe?.user;

    if (!isTelegramApp && !isDashboardRoute) {
      // Allow access for all dashboard routes, regardless of browser
      setRestrictAccess(true);
    } else if (isMobile && !isDashboardRoute && !isTelegramApp){
      setRestrictAccess(true);
    } else {
...
<div className="w-full flex justify-center">
        ...
        <div className="w-full flex justify-center">
          ...
          <TonConnectUIProvider manifestUrl="https://alphadogsgame.fun/tonconnect-manifest.json">
